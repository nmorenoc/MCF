...
void box::CreateSphere(int Ncurrent)
{
  ...
  while (counter<1000)
	...
	{
	  d=0.;
	  for (int k=0; k<DIM; k++) // check if overlapping other spheres
	    {
	  ...
	  for (int k=0; k<DIM; k++)    // check if overlapping wall
	    {     
	      if ((xrand[k] <= r) || (SIZE - xrand[k] <= r)) // touching wall
		{
		  keeper = 0;
		  counter++; 
		  break;
      ...
      else if (vi[k]>0)  // will hit right wall
	{
	  if (s[i].cell[k] == ngrids - 1)
	    newtime = ((double)(s[i].cell[k]+1)*SIZE/((double)(ngrids))
		       - (xi[k]+r+growthrate*gtime))/(vi[k]+growthrate);
	  else
	    newtime = ((double)(s[i].cell[k]+1)*SIZE/((double)(ngrids))
		     - xi[k])/(vi[k]);
	  if (newtime < 0)
			     ...
			     vector<DIM, int>& cpartnerpboffset)
{
  ...
  if (i!=j)
      ...
      if ((ctimej < ctime)&&(ctimej < s[j].nextevent.time))
	{
	  ...
	  if (pboffset.norm_squared() > 1E-15)
	    {
...
{  
  ...
  if ((e.j>=0)&&(e.j<N))  // collision!
      ...
      if (f.time <= e.time)
	{
	  ...
	  std::cout << "e.time= " << e.time << ", f.time= " << f.time << ", f.i= " << f.i << ", f.j= " << f.j << ", e.i= " << e.i << ", e.j= " << e.j <<std::endl;
	  std::cout << "difference= " << e.time - f.time << std::endl;
	  std::cout << "i's cell= " << s[i].cell << std::endl;
	  exit(-1);
	}
    }
}
