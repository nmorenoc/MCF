      ...
      !                                    the images for Lees-Edwards boundaries.
      !                                    
      ...
      !-------------------------------------------------------------------------
      !  Revision 1.14
      !  Include an optional variable shear_length, 
      !  which is used for Lees-Edwards boundary.
      !
    ...
    __KIND == __DOUBLE_PRECISION_COMPLEX
     ...
     &                 ppm_icommseq(i,ppm_topoid)
         ...
         !  ghost particles
         !----------------------------------------------------------------------
         IF ( lda > 1 .AND. PRESENT(shear_length) ) THEN
            CALL ppm_map_part_ghost_get(pdata,lda,Npart,isymm,ghostsize,info,shear_length(:,:))
         ELSE
